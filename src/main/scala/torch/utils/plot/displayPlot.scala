...
import geometry.{Drawable, Extent}
import torch.utils.plot.geometry.Placeable
import torch.utils.plot.service.Plot
import torch.utils.plot.service.aesthetics.Theme
import javax.swing.filechooser.FileNameExtensionFilter

/** Helper utilities for displaying utils.plot plots in their own window. The window offers
 * dynamic resizing for plots and the ability to save the plot as a PNG.
 *
 * This utility is published in the `utils.plot-repl` artifact, released alongside the
 * regular `utils.plot-jvm` artifact. It will need to be added to your build to use it.
 */
object displayPlot {
